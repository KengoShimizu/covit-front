...
// common
import { OwnerType, RouteName, RedirectFrom } from '../../common/Const';

export const UserRoute = ({ component: Component, ...rest }: any) => {
  const { uri } = useContext(RedirectContext);
  const { authState } = useContext(AuthContext);
  return (
    authState.isLogin && authState.user.is_owner === OwnerType.NOT_OWNER && uri.fromPath === RedirectFrom.ACCOUNTS ?
      <Redirect to={`${RouteName.HISTORY}?from=${uri.fromPath}`} />
